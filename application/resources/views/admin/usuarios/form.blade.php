...
@section('prev-url', route('clinica.usuarios.index'))
...
@section('topbar-options')
	<ul class="flex flex-auto flex-start flex-center">
		<li>
			<form @if (isset($row)) action="{{ go('clinica.usuarios.{id}.redefinir', $row->id) }}" @endif>
				...
				<form novalidate action="{{ route('clinica.usuarios.post') }}" method="post" autocomplete="off">
					...
					<div class="card">
						...
						<div class="card-content no-padding">
							...
							<div class="card-body fixed-height border-top scroller pt-3 pb-4" data-hide-x="true" style="height: calc(100vh - 340px);">
										...
										<div class="row">
											<div class="col s12 mb-1">
												<label class="left" style="cursor: pointer; display: flex; align-items: center;">
													...
													<span style="font-size: 1rem; margin-left: 20px; color: #000">Bloquear usuário</span>
												</label>
											</div>
										</div>
										<!-- END Input[Status] -->
