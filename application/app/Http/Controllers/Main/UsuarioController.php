...
namespace App\Http\Controllers\Main{
	...
	class UsuarioController extends Controller
	{
		...
		public function password_reset(Request $request, UsuarioModel $usuario)
		{

			$token = $request->token;

			if ($token = $usuario->getToken($token)) {

				// $decode           = base64_decode($token);
				$dados['request'] = $request;
				$dados['token']   = $token; // json_decode($token, true);

			} else {

				$dados['error'] = 'Token de segurança inválido';

			}

			return response(view('main.usuarios.passwords', $dados), 200);
