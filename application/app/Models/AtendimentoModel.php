...
class AtendimentoModel extends AppModel
{
	...
	public function getCategorias($data = null)
	{
		...
		$connection1 = env('DB_CONNECTION_1');
		$connection2 = env('DB_CONNECTION_2');

		$idioma = !isset($_COOKIE['idioma']) ? get_config('language') : $_COOKIE['idioma'];

		$get = $this->select(
			'id',
			DB::raw('(SELECT titulo FROM ' . DB::connection($connection1)->getDatabaseName() . '.tb_categoria_descricao WHERE id_categoria = id AND id_idioma =
				(SELECT id FROM ' . DB::connection($connection2)->getDatabaseName() . '.tb_sys_idioma WHERE sigla = "' . $idioma . '")
			) AS titulo')
		)
			->from(DB::connection($connection1)->getDatabaseName() . '.tb_categoria')
			->get();
	...
	public function getAtendimentos()
	{
		...
		$this->order = [
			...
			DB::raw('(SELECT `nome` FROM `tb_funcionario` AS `F` WHERE `F`.`id` = (SELECT `id_funcionario` FROM `tb_medico` AS `M` WHERE `M`.`id_funcionario` = `F`.`id` AND `M`.`id_funcionario` = (SELECT `id_medico` FROM `tb_medico_clinica` AS `C` WHERE `C`.`id_medico` = `M`.`id_funcionario` AND `C`.`id` = `A`.`id_medico`)))'),
