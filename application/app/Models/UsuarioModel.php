...
class UsuarioModel extends Authenticatable
{
	...
	public function getToken($token)
	{
		...
		$token   = json_decode(base64_decode($token->token), true);
	...
	public function send_password(Request $request)
	{
		...
		$id      = $request->id;
		$usuario = $this->select('email')->from('tb_acl_usuario')
			->where('id', $id)
			->first();
		...
		$data = json_encode([
			...
			'timestamp' => time(),
			'expires'   => $expires,
			'modulo'    => Session::get('userdata')[Session::get('app_session')]['id_modulo'],
		]);
		...
		$this->from('tb_acl_usuario')
			->where('id', $id)
			->update(['token' => $encode]);

		Mail::to($email)->send(new NewUserMail($request));

		return response()->json([
			'status'     => 'success',
			'message'    => 'Um link contendo a senha de redefinição foi enviado para o e-mail de cadastro do usuário.',
			'clean_form' => true,
			'reset_form' => true,
		]);
	...
	public function send_mail(Request $request)
	{
		...
		Mail::to($request->email)->send(new NewUserMail($request));
